...
def getAll():
  waiting_lines = WaitingLine.query.filter(and_(WaitingLine.company_id==g.user.company_id,WaitingLine.status == 1)).order_by(db.asc('name')).all()
...
def exitPosition(token):
  data = jwt.decode(token, current_app.config['SECRET_KEY'],
                  algorithms=['HS256'])
  line_up_id = data[ 'line_up_id']
  line_up = LineUp.query.filter(LineUp.id==line_up_id).first()
  line_up.status = 4
